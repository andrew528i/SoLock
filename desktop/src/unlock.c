...
static gboolean do_unlock(gpointer data)
{
    UnlockData *ud = data;

    SolockClient *client = solock_app_get_client(ud->app);
    GError *error = NULL;
    gboolean ok = solock_client_unlock(client, ud->password, ud->timeout, &error);
...
static void on_unlock_activate(GtkEntry *widget, gpointer data)
{
    ...
    gtk_widget_set_sensitive(ud->entry, FALSE);

    SolockConfig *config = solock_app_get_config(ud->app);
    ud->password = g_strdup(password);
    ud->timeout = solock_config_get_timeout_minutes(config);

    g_timeout_add(50, do_unlock, ud);
}
...
static void on_view_map(GtkWidget *widget, gpointer data)
{
    (void)widget;
    UnlockData *ud = data;
    gtk_widget_set_sensitive(ud->entry, TRUE);
